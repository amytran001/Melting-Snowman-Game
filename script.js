...
function updateMeltingSnowmanGraphic() {
    ...
    meltingSnowmanContainer.innerHTML = `<img src="images/MeltingSnowman${imageCount}.png" alt="Melting Snowman ${imageCount}">`;
    imageCount++;
}
...
function checkWinOrLose() {
    if (guessedLetters.join('') === wordToGuess) {
        ...
        const meltingSnowmanContainer = document.querySelector('.melting-snowman');
        meltingSnowmanContainer.innerHTML = `<img src="gameover.png" alt="Game Over">`;
        const letterButtons = document.querySelectorAll('.letters button');
        letterButtons.forEach(button => {
            button.disabled = true;
            button.removeEventListener('click', handleGuess);
